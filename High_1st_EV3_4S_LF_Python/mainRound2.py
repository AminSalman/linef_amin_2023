...
PID_speed, turnSpeed2, straightSpeed = 85, 81, 100  # PID speed, hard turns speed, and straight speed.
...
while True:
    ...
    if sensorState in [1,3,7]:	# Check if list contains current state
        ...
        if time>3.8 and time<9: # Check if time is between 3.2 and 8 seconds
            turnSpeed = 100     # Then set hard turning speed to maximum
        else:
            turnSpeed = turnSpeed2   # Otherwise, set it to a low turning speed
        ...
        if time>3.2 and time<8: # Check if time is between 3.2 and 8 seconds
            turnSpeed = 100     # Then set hard turning speed to maximum
        else:
            turnSpeed = turnSpeed2   # Otherwise, set it to a low turning speed
